...
###########################################################################
get '/' do
  #このSNSアプリではルートディレクトリでは特にプログラムを動かしておらず、静的ファイルのみを表示しているため、処理を書いていない
  erb :index
end
...
get '/login' do
  ...
  redirect '/posts' if session[:id] #session[:id]があるときのみ実行される
  erb :login
end
...
get '/posts/:id' do
  ...
  #投稿に対するコメントをwhereで取得し変数へ代入、to_charは50目付近の説明を参照
...
get '/search' do
  ...
  @keywords.each_with_index do |keyword| #複数のキーワードで検索するため、キーワードをeachで回して処理を行う
    ...
    array.each do |a| #上記で検索結果が配列で帰ってくるため更にeachでヒットしたデータを順番に取り出す
      ...
      @results << a #@results変数に検索結果を追加する @resultsは配列として用意しているので、 << を使うことで、配列に値を追加することができる
    end
  end
